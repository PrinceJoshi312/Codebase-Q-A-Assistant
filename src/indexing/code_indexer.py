...
class CodeIndexer:
    ...
    def index_files(self, file_paths: List[Path]):
        ...
        print(f"[*] Starting Neural Ingestion for {total_files} files...")
        
        # Use ThreadPoolExecutor for parallel LLM calls (Feature 2 optimization)
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            future_to_file = {executor.submit(process_file, file_path, i+1, total_files): file_path for i, file_path in enumerate(file_paths)}
